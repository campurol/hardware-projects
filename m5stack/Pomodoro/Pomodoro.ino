/**
  ...
  curl -X POST -H "Content-Type: application/json" -d '{"value1":"30"}' https://maker.ifttt.com/trigger/post/with/key/b_xypTQ65svHWRoy6d3TN0
...
#define IFTTT_URL "https://maker.ifttt.com/trigger/post/with/key/b_xypTQ65svHWRoy6d3TN0"
#define WIFI_SSID "Patagonia"
#define WIFI_PASSWORD "@patagoniasecret"
...
void loop() {
  ...
  if (rest_state == false) {
    ...
    if (state != 1 && state != 3 && state != 4  && IMU.ay > acc_th) {
      ...
      if (state != 4) {
        state = 4;
        //// here is where I should put the setup of the pomodoro
        /// number of minutes of each chunk
        /// number of chunk goal
        pomodoro_state();
      }
      else
      {
...
void pomodoro_state() {
  ...
  M5.Lcd.print(String(pomodoro_times) + " pomochuncks of " + String(pomodoro_minutes) + " min");
  M5.Lcd.setTextSize(10);
  ...
  while (i < LCD_MAX_Y) {
    if (millis() >= pomodoro_time + time_fraction) {
      ...
      pomodoro_times = pomodoro_times - 2;
      M5.Lcd.clear();
      M5.Lcd.setTextSize(10);
      M5.Lcd.setCursor(60, 90);
  ...
  pomodoro_times = pomodoro_times + 1;
  M5.Lcd.clear();
  M5.Lcd.setRotation(1);
  M5.Lcd.setCursor(60, 90);
  M5.Lcd.setTextColor(GREEN);
  ...
  #ifdef ARDUINO_M5STACK_FIRE
    for (int pixelNumber = 0; pixelNumber < 10; pixelNumber++) {
      leds[pixelNumber].setRGB(bright_leds, 0, bright_leds);
    }
    FastLED.show();
  #endif
}
